 ...
 */
validate.validators.conditional = function (value, options) {
...
function jumpToError(event, name) {
  ...
  const targetElement =
    document.getElementById(nameToId(name)) ||
    document.getElementById(`${nameToId(name)}-items`)
...
function showErrorSummary(errors) {
  ...
  // Create a new error summary list
  const errorSummaryBody = nhsukErrorSummary.querySelector(
    '.nhsuk-error-summary__body'
  )
  const errorList = document.createElement('ul')
  errorList.classList.add('nhsuk-list', 'nhsuk-error-summary__list')
